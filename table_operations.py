...
def insert_data(cursor, table_name, json_obj):
    columns = [key.lower() for key in json_obj.keys() if '$' not in key and key != "_id"]
    values = [process_value(value) for key, value in json_obj.items() if '$' not in key and key != "_id"]
    if '_id' in json_obj:
        columns.append('_id')
